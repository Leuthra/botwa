...
export class ShinigamiService {
  ...
  mapMangaItem(item: any) {
    ...
    return {
      ...
      updated: item.latest_chapter_time || item.updated_at || "",
      rating: item.user_rate || 0,
  ...
  async getPopularManga(page = 1) {
    try {
      ...
      if (error.response) {
        throw new Error(
          `Failed to get popular manga: ${error.response.status} - ${error.response.data?.message || error.message}`,
        );
      }
  ...
  async getLatestUpdates(page = 1) {
    try {
      ...
      if (error.response) {
        throw new Error(
          `Failed to get latest updates: ${error.response.status} - ${error.response.data?.message || error.message}`,
        );
      }
  ...
  async searchManga(query: string, page = 1) {
    try {
      ...
      if (error.response) {
        throw new Error(
          `Failed to search manga: ${error.response.status} - ${error.response.data?.message || error.message}`,
        );
      }
  ...
  async getMangaDetails(mangaId: string) {
    try {
      ...
      if (error.response) {
        throw new Error(
          `Failed to get manga details: ${error.response.status} - ${error.response.data?.message || error.message}`,
        );
      }
  ...
  async getChapterList(mangaId: string) {
    try {
      ...
      const chapters = chapterList.map((item: any) => {
        let dateUpload = 0;
        ...
        return {
          ...
          chapterUrl: `${this.baseUrl}/series/${mangaId}/${item.chapter_id || item.id}`,
        };
      });
      ...
      if (error.response) {
        throw new Error(
          `Failed to get chapter list: ${error.response.status} - ${error.response.data?.message || error.message}`,
        );
      }
  ...
  async getPageList(chapterId: string) {
    try {
      ...
      if (error.response) {
        throw new Error(
          `Failed to get page list: ${error.response.status} - ${error.response.data?.message || error.message}`,
        );
      }
