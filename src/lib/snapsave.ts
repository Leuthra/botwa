...
export const snapsave = async function (url: string) {
  try {
    const bodyReq = new URLSearchParams();
    bodyReq.append("url", url);
    ...
    if (htmlContentMatch && htmlContentMatch[1]) {
      ...
      if ($("div.download-items").length) {
        $("div.download-items").each((_, el) => {
          const $el = $(el);
          const dlUrl = $el.find(".download-items__btn > a").attr("href");
          const rawThumb = $el.find(".download-items__thumb > img").attr("src");
          let thumb = rawThumb;
          if (rawThumb && rawThumb.startsWith("http")) {
            const thumbUrl = new URL(rawThumb);
            thumb = thumbUrl.searchParams.get("photo") || thumbUrl.toString();
          }
          if (dlUrl) {
            results.push({
              thumbnail: thumb,
        ...
        $("tbody > tr").each((_, el) => {
          ...
          if (_url && /get_progressApi/gi.test(_url)) {
            shouldRender = true;
        ...
        const dlUrl = $("div.column > a").attr("href");
        if (dlUrl) {
          results.push({
            thumbnail,
            url: dlUrl,
    ...
    return {
      ok: false,
      message: e.message || "Error occurred during scraping",
    };
  }
};

function decryptSnapSaveData(obfuscatedDataString: string) {
  try {
    const parts = obfuscatedDataString.split("decodeURIComponent(escape(r))}(");
    if (!parts[1]) return "";
    ...
    function customBaseDecode(
      ...
      let decimalValue = encodedString
        ...
        .reduce((accumulator, char, index) => {
          if (inputChars.includes(char)) {
            return (
              accumulator +
              inputChars.indexOf(char) * Math.pow(inputBase, index)
            );
          }
          return accumulator;
        }, 0);
      ...
      while (decimalValue > 0) {
        ...
        decimalValue =
          (decimalValue - (decimalValue % outputBase)) / outputBase;
      }
      return result || "0";
    }
    ...
    for (let i = 0; i < h.length; i++) {
      let segment = "";
      while (h[i] !== n[parseInt(e)] && i < h.length) {
        segment += h[i];
        i++;
      }
      ...
      decryptedString += String.fromCharCode(
        parseInt(customBaseDecode(segment, parseInt(e), 10)) - parseInt(t),
      );
    }
